...
export default function Home() {
  ...
  const playoffBracket = useMemo(() => {
    ...
    const sf1 = playoffMatch("sf1", winnerOf("qf1"), winnerOf("qf3"));
    const sf2 = playoffMatch("sf2", winnerOf("qf2"), winnerOf("qf4"));
  ...
  const playoffFixtureTitle = (key: string, format?: "internal" | "external") => {
    if (format === "internal") {
      ...
      if (key === "q2") return "Qualifier 2";
      if (key === "final") return "Final";
      return key;
    }
    if (key === "qf1") return "QF1 (1 vs 8)";
    if (key === "qf2") return "QF2 (2 vs 7)";
    if (key === "qf3") return "QF3 (3 vs 6)";
    if (key === "qf4") return "QF4 (4 vs 5)";
    if (key === "sf1") return "SF1 (QF1 vs QF3)";
    if (key === "sf2") return "SF2 (QF2 vs QF4)";
    if (key === "final") return "Final";
    if (key === "bronze") return "Bronze match";
    return key;
  };
  ...
  const completeTournament = async (tournament: Tournament) => {
    ...
    setNotice(`🏆 ${tournament.name} is now marked as Completed! All playoff results are locked.`);
    if (hasSupabaseConfig && supabase) {
      const { error } = await supabase.from("tournaments").update({ status: "completed" }).eq("id", tournament.id);
      if (error) {
        setNotice(`Tournament completed locally. (Supabase note: ${error.message} - please run the RLS migration to sync publicly).`);
        return;
      }
      await refreshDatabase();
    }
  ...
  const reopenTournament = async (tournament: Tournament) => {
    setTournaments((current) => current.map((item) => (item.id === tournament.id ? { ...item, status: "active" } : item)));
    setNotice(`Reopened ${tournament.name}.`);
    if (hasSupabaseConfig && supabase) {
      const { error } = await supabase.from("tournaments").update({ status: "active" }).eq("id", tournament.id);
      if (error) {
        setNotice(`Reopened locally. Supabase note: ${error.message}`);
        return;
      }
      await refreshDatabase();
    }
            ...
            <nav className="flex flex-wrap items-center gap-2">
              {[
                ...
                <button
                  key={item.key}
                  type="button"
                  disabled={item.key === "playoffs" && !matchDraft.tournamentId}
                  onClick={() => { if (item.key !== "playoffs" || matchDraft.tournamentId) setTab(item.key as TabName); }}
                  className={`nav-pill rounded-full px-4 py-2 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-45 ${
                    tab === item.key
              ...
              {isAuthenticated ? <button type="button" onClick={() => void handleLogout()} className="nav-pill rounded-full bg-[#f7f8fa] px-4 py-2 text-sm font-medium text-[#18324f]">Log out</button> : <button type="button" onClick={() => setShowLogin(true)} className="nav-pill rounded-full bg-[#f7c62f] px-4 py-2 text-sm font-semibold text-[#071a2d]">Admin login</button>}
            </nav>
          </div>
        </header>

        <main data-playoff-complete={playoffComplete ? "true" : "false"} className="space-y-6">
          {notice && notice !== "Connected to Supabase. Live sync is active." && (
            <div className="flex items-center justify-between rounded-2xl border border-[#f7c62f]/40 bg-[#0d2b4a] px-4 py-3 text-sm text-[#fce6a0] shadow-md">
              <span className="font-medium">{notice}</span>
              <button
                type="button"
                onClick={() => setNotice("")}
                className="ml-3 rounded-full border border-white/20 bg-white/10 px-2.5 py-0.5 text-xs text-slate-300 hover:bg-white/20"
              >
                Dismiss
              </button>
            </div>
          ...
          {tab === "dashboard" && (
            <>
              ...
              <section className="rounded-[26px] border border-[#d9d3d0] bg-[#f9f7f5] p-5">
                ...
                <div className="mt-5">
                  <select value={fixtureTeamFilter} onChange={(event) => setFixtureTeamFilter(event.target.value)} className="w-full rounded-xl border border-[#ded8d4] bg-white px-3 py-2.5 text-sm text-[#17191d] outline-none"><option value="">All teams ({tournamentFixtures.length} fixtures)</option>{tournamentTeams.map((team) => <option key={team.id} value={team.id}>{teamLabel(team)}</option>)}</select>
                </div>
                <div className="mt-5 grid gap-3 md:grid-cols-2">
                  {filteredFixtures.map((fixture) => <div key={`${fixture.teamA.id}-${fixture.teamB.id}`} className="rounded-[18px] border border-[#e4dfdc] bg-white p-4"><div className="flex items-center justify-between gap-3"><p className="font-semibold text-[#17191d]">{teamLabel(fixture.teamA)} <span className="text-[#8a9097]">vs</span> {teamLabel(fixture.teamB)}</p><span className={`rounded-full px-2 py-1 text-[10px] uppercase tracking-[0.14em] ${fixture.match ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{fixture.match ? "Completed" : "Pending"}</span></div><p className="mt-3 text-sm text-[#626972]">{fixture.match ? `${fixture.teamAScore} : ${fixture.teamBScore}` : "No score yet"}</p>{!playoffComplete && fixture.match && <button type="button" onClick={() => editMatch(fixture)} className="mt-3 rounded-full border border-[#cdd8f7] px-3 py-1.5 text-xs font-semibold text-[#3949ab]">Edit result</button>}</div>)}
                      ...
                      isPlayoffTournamentCompleted ? (
                        <div className="flex items-center gap-2">
                          ...
                          {isAuthenticated && (
                            <button
                              type="button"
                              onClick={() => void reopenTournament(playoffTournament)}
                              className="rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-white/20"
                            >
                              Reopen
                            </button>
                          )}
